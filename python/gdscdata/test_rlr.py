...
# A modification of 'robust private linear regression' where no privacy is implemented.
# Precision measures: 
...
for seed in range(n_cv):
	...
	csvpath = datapath+'cliptest-drugsens-rlr-corr-'+stre+'-'+str(drugid)+'-'+str(seed)+'.csv'
	np.savetxt(csvpath,S,delimiter=',')
	csvpath = datapath+'cliptest-drugsens-rlr-wpc-'+stre+'-'+str(drugid)+'-'+str(seed)+'.csv'
