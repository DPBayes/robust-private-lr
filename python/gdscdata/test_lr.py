...
# Modification of 'robust private linear regression' where no clipping is done and no privacy is implemented.
# Precision measures: 
...
for seed in range(n_cv):
	...
	for i in range(len(pv_size)):
		...
		# Process data
		nxx_pv,nxx_npv,nxy_pv,nxy_npv,nyy_pv,nyy_npv,x_test,y_test,B_x,B_y,n_train,private = dp.processData(x,y,d,n_test,n_pv,n_npv,pv_max,w_x,w_y,drugid,seed,clipdata=False) # modification: lr
	...
	csvpath = datapath+'cliptest-drugsens-lr-corr-'+stre+'-'+str(drugid)+'-'+str(seed)+'.csv'
	np.savetxt(csvpath,S,delimiter=',')
	csvpath = datapath+'cliptest-drugsens-lr-wpc-'+stre+'-'+str(drugid)+'-'+str(seed)+'.csv'
