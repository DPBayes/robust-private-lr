...
def precision(y_pred,y_real):
	...
	if np.isnan(r):
		...
		return r
"""
# Precision measure: mean squared error relative to the variance of the original data
def precision(y_pred,y_real):
	return np.mean((y_real-y_pred)**2) # TODO: !!! relative to variance
"""
"""
...
def omega(n,d,eps,mcmc,ln=20,p1=0.35,p2=0.6,p3=0.05):
	...
	# Choose best
	ind = np.unravel_index(err.argmax(),err.shape) # TODO: choose EITHER mse: argmin; OR spearman: argmax
	w_x = st[ind[0]]
	w_y = st[ind[1]]
	...
	if plotting_on:
		...
		for label in ax.get_yticklabels():
			if i%2 != 0:
				label.set_visible(False)
			i = i+1
		plt.title('Average MSE on a synthetic data set')
		plt.show()
